...
function parseTutorialMarkdown(content, tutorialKey) {
  ...
  for (const line of lines) {
    ...
    if (currentSection === 'problems' && currentProblem) {
      if (trimmed.startsWith('**Premises:**')) {
        ...
        // Check if conclusion is on the same line
        const conclusionText = trimmed.substring('**Conclusion:**'.length).trim();
        if (conclusionText) {
          currentProblem.conclusion = conclusionText;
          currentField = '';
        }
        continue;
      } else if (trimmed.startsWith('**Brief Solution:**')) {
        currentField = 'solution';
        // Check if solution is on the same line
        const solutionText = trimmed.substring('**Brief Solution:**'.length).trim();
        if (solutionText) {
          currentProblem.solution = solutionText;
          currentField = '';
        }
        continue;
      ...
      if (currentField === 'premises') {
        ...
        if (!trimmed.startsWith('**') && trimmed !== '') {
          ...
          currentField = ''; // Reset after capturing conclusion
        }
      } else if (currentField === 'solution') {
        if (!trimmed.startsWith('**') && trimmed !== '') {
          currentProblem.solution = trimmed;
          currentField = ''; // Reset after capturing solution
        }
      }
    }
    // Parse description
...
function extractClassicProblems() {
